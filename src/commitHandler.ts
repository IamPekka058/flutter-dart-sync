...
async function commitChanges(pubspecPath: string): Promise<void> {
    ...
    if(commitMessage === '' || gh_app_id === '' || gh_app_installation_id === '' || gh_app_private_key === ''){
        core.warning('One or more required inputs are missing regarding commit. Skipping commit.');
        return;
    }

    const auth = createAppAuth({appId: gh_app_id, privateKey: gh_app_private_key, installationId: gh_app_installation_id});
    const installationAuth = await auth({type: 'installation', installationId: gh_app_installation_id});
